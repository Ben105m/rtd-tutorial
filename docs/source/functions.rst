...
PyDrag is manipulating different objects, each containing different kinds of informations :
  ...
  - `Library object <library>` : contains all the available informations for the chosen nuclear data library. The user does not need to directly interact with it, as the library type, the isotopes contained inside, and the nuclear data are automatically extracted with PyDrag ;
